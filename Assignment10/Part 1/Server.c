...
bzero((char *) &dns_req, sizeof(DNS));
n=read(newsockfd, &dns_req, sizeof(DNS));
if (n<0) error("ERROR reading from socket DNS");
printf("recv DNS req - ID: %d, Domain: %s\n", dns_req.id, dns_req.qname);
 
 
 
FILE *file = fopen("dns_records.txt", "r");
char file_domain[256], file_ip[16];
int found = 0;
strcpy(dns_req.ipaddr,"NOT_FOUND");

if (file) {
    while (fscanf(file, "%s %s", file_domain, file_ip) != EOF) {
        if (strcmp(file_domain, dns_req.qname) == 0) {
            strcpy(dns_req.ipaddr, file_ip);
            found = 1;
            break;
        }
    }
    ...
    perror("ERROR opening dns_records.txt");
}
 
 
n=write(newsockfd, &dns_req, sizeof(DNS));
