...
main (int argc, char *argv[])
{
        ...
        //making a conditional check to set the req algorthm  atq

        // std::string tcpva

  std::string tcpVariant = "TcpNewReno";
  std::string linkBw = "5Mbps";
  std::string linkDelay = "2ms";

  CommandLine cmd;
  //
  cmd.AddValue("tcpVariant", "Which tcp variant ", tcpVariant);
  cmd.AddValue("linkBw", "  bandwidth", linkBw);
  cmd.AddValue("linkDelay", " latency/ delay", linkDelay);
  cmd.Parse(argc, argv);
  ...
  if (tcpVariant == "TcpNewReno") {
  ...
  else if (tcpVariant == "TcpHighSpeed") {
    ...
    Config::SetDefault("ns3::TcpL4Protocol::SocketType", TypeIdValue(TcpVegas::GetTypeId()));
        ...
        pointToPoint.SetDeviceAttribute ("DataRate", StringValue (linkBw));            //set the data rate and delay here
